...
func createCustomer(db *sql.DB, c *gin.Context) (int, *Customer, error) {
	...
	err := db.QueryRow(stmt, customer.Name, customer.Email, customer.Address).Scan(&customer.ID)
	if err != nil {
...
func getCustomer(db *sql.DB, c *gin.Context) (int, *Customer, error) {
	...
	err = db.QueryRow(stmt, id).Scan(&customer.ID, &customer.Name, &customer.Email, &customer.Address)
...
func updateCustomer(db *sql.DB, c *gin.Context) (int, *Customer, error) {
	customerID := c.Param("customerId")
	id, err := strconv.Atoi(customerID)
	if err != nil {
		return http.StatusBadRequest, nil, err
	...
	err = db.QueryRow(stmt, fields...).Scan(&customer.ID, &customer.Name, &customer.Email, &customer.Address)
	if err != nil {
		return http.StatusInternalServerError, nil, err
	}
...
func deleteCustomer(db *sql.DB, c *gin.Context) (int, error) {
	customerID := c.Param("customerId")
	id, err := strconv.Atoi(customerID)
	if err != nil {
